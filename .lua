...
local function getPlayerHIDDENINSTANCE(str,funct)
  for i,v in pairs(str:GetChildren()) do
    funct(v)
  end
end

local function IsA(v,str)
  return v:IsA(str)
end
...
function ESPLib:CreateESPTag(params)
    ...
    running = RunService.RenderStepped:Connect(updateesplabelfr)
    function ESPLib:RunBreak()
      if running then
       running:Disconnect()
       getPlayerHIDDENINSTANCE(Part,function(v)
          if IsA(v,"Highlight") and IsA(v,"BillboardGui") and IsA(v,"TextLabel") and IsA(v,"BoxHandleAdornment") and IsA(v,"Trail") and IsA(v,"Attachment") then
            v:Destroy()
          end
       end)
      end
